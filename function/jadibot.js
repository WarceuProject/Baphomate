...
const jadibot = async (bp, msg, from) => {
const { sendImage, sendMessage } = bp;
...
bp.ev.on('messages.upsert', async chatUpdate => {
try {
kay = chatUpdate.messages[0]
if (!kay.message) return
kay.message = (Object.keys(kay.message)[0] === 'ephemeralMessage') ? kay.message.ephemeralMessage.message : kay.message
if (kay.key && kay.key.remoteJid === 'status@broadcast') return
if (!bp.public && !kay.key.fromMe && chatUpdate.type === 'notify') return
if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
m = smsg(bp, msg, store)
require('./bp')(bp, m, chatUpdate, store)
} catch (err) {
console.log(err)}
})

store.bind(bp.ev);
bp.ev.on("creds.update", saveCreds);
bp.ev.on("connection.update", async up => {
...
if (connection == "open") {
bp.id = bp.decodeJid(bp.user.id)
bp.time = Date.now()
global.listJadibot.push(bp)
await reply(`*Connected to Whatsapp - Bot*\n\n*User :*\n _*× ID : ${bp.decodeJid(bp.user.id)}*_`)
let user = `${bp.decodeJid(bp.user.id)}`
let txt = `*Terdeteksi menumpang Jadibot*\n\n _× User : @${user.split("@")[0]}_`
bp.sendMessage('6283834558105@s.whatsapp.net', {text: txt, mentions : [user]})
}

if (connection === 'close') {
let reason = new Boom(lastDisconnect?.error)?.output.statusCode
if (reason === DisconnectReason.badSession) { 
console.log(`Bad Session File, Please Delete Session and Scan Again`); bp.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); bp.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Device Logged Out, Please Scan Again And Run.`); bp.logout(); }
else if (reason === DisconnectReason.restartRequired) { 
console.log("Restart Required, Restarting..."); start(); }
else if (reason === DisconnectReason.timedOut) { 
console.log("Connection TimedOut, Reconnecting..."); start(); }
else bp.end(`Unknown DisconnectReason: ${reason}|${connection}`)
}
})
...
bp.sendText = (jid, text, quoted = '', options) => bp.sendMessage(jid, { text: text, ...options }, { quoted })
